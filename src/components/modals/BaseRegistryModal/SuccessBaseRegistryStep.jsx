...
export const SuccessBaseRegistryStep = ({ closeModal, upgradedLevel }) => {
  ...
  return (
    <div className="p-10 flex z-10 relative flex-col justify-center items-center w-full bg-black-light sm:bg-main-bg rounded sm:rounded-none sm:p-5 sm:w-full sm:h-full ">
      <div className="flex flex-col justify-center items-center sm:flex-1">
        ...
        <span className="text-white-500 mt-7 text-base text-center sm:text-sm">
          ...
          <span className="text-white">
            {t('smartGameProLevel')} {upgradedLevel} {capitalize(t('activated'))}
          </span>
        </span>
      ...
      <Button
        className="w-full text-white-500 font-medium mt-4 text-xs sm:text-base"
        type={BUTTON_TYPES?.TRANSPARENT}
        onClick={closeModal}
      >
        <CloseIcon className="w-4 h-4 stroke-current text-white-500 mr-1 sm:w-5 sm:h-5" />
        {capitalize(t('close'))}
      </Button>
    </div>
  );
